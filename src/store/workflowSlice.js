  ...
  reducers: {
    ...
    addNode: (state, action) => {
      ...
      if (parentNodeId && state.nodes[parentNodeId]) {
        ...
        if (state.nodes[parentNodeId].type === 'branch' && action.payload.branchLabel) {
          if (!state.nodes[parentNodeId].branchLabels) {
            state.nodes[parentNodeId].branchLabels = {};
          }
          state.nodes[parentNodeId].branchLabels[newNodeId] = action.payload.branchLabel;
        }
      }
    },
    ...
    deleteConnection: (state, action) => {
      ...
      if (connection) {
        ...
        if (parentNode) {
          ...
          if (parentNode.branchLabels && parentNode.branchLabels[connection.toNodeId]) {
